...
class CareTypeAdmin(admin.ModelAdmin):
    """
    Админ-панель для модели CareType.
    
    Позволяет управлять типами ухода за растениями.
    Поддерживает фильтрацию, поиск и сортировку записей.
    """
...
class CareTaskAdmin(admin.ModelAdmin):
    """
    Админ-панель для модели CareTask.
    
    Управляет задачами по уходу за растениями.
    Отображает текущие и предстоящие задачи, поддерживает автозаполнение растений,
    фильтрацию по статусу и датам, а также вычисляет следующую дату выполнения.
    """
    ...
    def get_queryset(self, request):
        """
        Оптимизирует запрос, добавляя select_related для полей plant и care_type,
...
class CareTaskLogAdmin(admin.ModelAdmin):
    """
    Админ-панель для модели CareTaskLog.
    
    Отображает журнал выполнения задач по уходу.
    Запрещает добавление и удаление записей через админку.
    Ограничивает отображение заметок первыми 50 символами.
    """
    ...
    def notes_preview(self, obj):
        """Возвращает укороченную версию заметок (первые 50 символов) с многоточием."""
